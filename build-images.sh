...
container=$(buildah from docker.io/alpine:3.23.0)
...
if ! buildah containers --format "{{.ContainerName}}" | grep -q nodebuilder-crowdsec; then
    ...
    buildah from --name nodebuilder-crowdsec -v "${PWD}:/usr/src:Z" docker.io/library/node:24-slim
fi
...
buildah config --entrypoint=/ \
    ...
    --label="org.nethserver.images=docker.io/crowdsecurity/crowdsec:v1.7.3-debian ${repobase}/crowdsec-firewall-bouncer:${IMAGETAG:-latest}" \
